...
jeedom.loadCSS = function(_href) {
  const existing = Array.from(document.querySelectorAll('link[rel="stylesheet"]')).find(function(_link) {
    const href = _link.getAttribute('href') || ''
    return href == _href || href.startsWith(_href + '?') || _link.href.endsWith('/' + _href) || _link.href.includes('/' + _href + '?')
  })
  ...
  return new Promise(function(resolve, reject) {
    ...
    if (typeof domUtils !== 'undefined' && domUtils.headInjexted) {
      domUtils.headInjexted.push(link.href)
    }
  })
}
