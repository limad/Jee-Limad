...
if (!jeeFrontEnd.massedit) {
  jeeFrontEnd.massedit = {
    ...
    addFilter: function() {
      ...
      let newFilter = document.querySelectorAll('#filter > div.filter').last()
    ...
    getFilters: function() {
      ...
      document.querySelectorAll('#filter > div.filter').forEach(_filterEl => {
        ...
        if (!(_filterEl.querySelector('.selectFilterJValue').disabled)) {
          jValue = _filterEl.querySelector('select.selectFilterJValue').selectedOptions[0].text
        }
        filters.push({
          'key': key,
          'value': value,
    ...
    getEdits: function() {
      ...
      document.querySelectorAll('#edit > .edit').forEach(function(edit) {
        ...
        if (!edit.querySelector('.inputEditJValue').disabled) {
          jValue = edit.querySelector('.inputEditJValue').value
        }
        edits.push({
          'key': key,
          'value': value,
    ...
    execMassEdit: function() {
      ...
      let sqlCmd = jeeP.getTestSQLstring(filters)
