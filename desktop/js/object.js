  ...
  jeeFrontEnd.object = {
    ...
    saveObject: function() {
      ...
      jeedom.object.save({
        ...
        success: function(data) {
          jeeFrontEnd.modifyWithoutSave = false
          let url = 'index.php?v=d&p=object&id=' + data.id + '&saveSuccessFull=1'
      ...
      return {
        callback: function(key, options, event) {
          if (event.ctrlKey || event.metaKey || event.which == 2) {
            let url = 'index.php?v=d&p=object&id=' + options.commands[key].id
            if (window.location.hash != '') {
              url += window.location.hash
            }
...
document.getElementById('div_conf').addEventListener('click', function(event) {
  ...
  if (_target = event.target.closest('.listCmdInfo')) {
    let el = _target.closest('.summary').querySelector('.summaryAttr[data-l1key="cmd"]')
...
document.getElementById('div_conf').addEventListener('change', function(event) {
  ...
  if (_target = event.target.closest('ul.dropdown-menu input[type="checkbox"]')) {
    ...
    let el = document.querySelector('.type' + type)
    const summary = {
      cmd: cmd,
      enable: "1",
      invert: "0"
