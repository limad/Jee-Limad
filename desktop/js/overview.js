...
if (!jeeFrontEnd.overview) {
  jeeFrontEnd.overview = {
    ...
    updateSummary: function(_className) {
      ...
      if (!_classEl) return
      const parent = _classEl.closest('.objectPreview')
...
jeeP.modalContent.addEventListener('click', function(event) {
  ...
  if (event.target.matches('div.eqLogic-widget .history') || event.target.closest('.cmd-widget.history') != null ) { //history in summary modal
    ...
    if (event.ctrlKey || event.metaKey) {
      ...
      cmdIds = event.target.closest('.history[data-cmd_id]').getAttribute('data-cmd_id')
    }
    jeeDialog.dialog({
      id: 'md_cmdHistory',
      title: '{{Historique}}',
...
document.getElementById('div_pageContainer').addEventListener('click', function(event) {
  ...
  if (_target = event.target.closest('.objectSummaryParent')) {
    ...
    let title
    if (icon) {
      title = icon.outerHTML + ' ' +  _target.closest('.objectPreview').querySelector('.topPreview .name').textContent
    } else {
      title = _target.closest('.objectPreview').querySelector('.name').textContent
    }
