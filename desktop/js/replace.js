  ...
  jeeFrontEnd.replace = {
    ...
    selectReplacerEqlogic: function(_selectEl) {
      ...
      cmds.forEach( _cmd => {
        ...
        let optionsCmds
        if (type == 'info') {
          optionsCmds = replacerCmdsInfo
        } else {
          optionsCmds = replacerCmdsAction
        }

        cmdSelect = _cmd.querySelector('select')
        cmdSelect.empty()
        let options = '<option value=""></option>'
        let cmdData = jeephp2js.listCommands.filter(o => o.id == _cmd.getAttribute('data-id'))[0]
        optionsCmds.forEach(function(optionsCmd) {
          if (cmdData.name.toLowerCase().trim() == optionsCmd.name.toLowerCase().trim() && cmdData.type == optionsCmd.type) {
