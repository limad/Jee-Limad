...
document.getElementById('bt_remove')?.addEventListener('click', function(event) {
  ...
  jeedom.report.remove({
    ...
    success: function(data) {
      ...
      const activeTypeNumber = document.querySelector('.li_reportType.active .number')
      if (activeTypeNumber != null) activeTypeNumber.textContent = Number(activeTypeNumber.textContent) - 1
    }
  })
})
...
document.getElementById('bt_removeAll')?.addEventListener('click', function(event) {
  jeedom.report.removeAll({
    ...
    success: function(data) {
      ...
      const activeTypeNumber = document.querySelector('.li_reportType.active .number')
      if (activeTypeNumber != null) activeTypeNumber.textContent = '0'
    }
  })
})
