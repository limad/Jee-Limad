      ...
      if (_update.type != 'core') {
        if (_update.configuration && _update.configuration.version == 'beta') {
          if (isset(_update.plugin) && isset(_update.plugin.changelog_beta) && _update.plugin.changelog_beta != '') {
            ...
            tr += '<button type="button" class="btn btn-xs disabled"><i class="fas fa-book"></i><span class="hidden-1280"> {{Changelog}}</span></button> '
          }
        } else {
...
document.getElementById('div_pageContainer').addEventListener('click', function(event) {
  ...
  if (_target = event.target.closest('#table_update input[data-l2key="doNotUpdate"]')) {
    ...
    if (_target.checked) {
      _target.closest('tr').querySelector('.btn.update').addClass('disabled')
    } else {
      _target.closest('tr').querySelector('.btn.update').removeClass('disabled')
    }
    return
  }
